...
func TestNetworkPipeline_Integration(t *testing.T) {
	var (
		mu       sync.Mutex
		received int
		lastBody []byte
	...
	if ev.Action != "NETWORK_CONNECTION_SNAPSHOT" {
		t.Errorf("Expected action NETWORK_CONNECTION_SNAPSHOT, got %s", ev.Action)
	}
