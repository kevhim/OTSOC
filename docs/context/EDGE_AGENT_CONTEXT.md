...
- `device_id` != `asset_id` (Device ID is the unique hardware/installation identifier, Asset ID refers to the monitored OT asset).
...
- `seq_no` is per-device ordering/gap-detection state. It is strictly monotonic per device.
