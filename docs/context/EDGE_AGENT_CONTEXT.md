...
- **Architectural Gap (Canonical Event Identity):** `TenantID`, `SiteID`, and `AssetID` are required by CanonicalEvent schema validation. Currently, `agent/cmd/endpoint/main.go` parses these values from configuration but lacks a centralized enrichment point or middleware to inject them into collector-produced raw events before `Storage.Store()`. Collectors intentionally output unenriched canonical payloads to avoid duplicating enrichment logic in each collector. A structured enrichment stage must be established in the core pipeline before 2C goes to production.
